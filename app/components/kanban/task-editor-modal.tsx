...
export function TaskEditorModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-950/20 p-6 backdrop-blur-sm dark:bg-zinc-950/60">
      <Card className="w-full max-w-lg overflow-hidden border-white/40 p-0 shadow-2xl shadow-zinc-950/20 dark:border-zinc-800 dark:shadow-none">
        <div className="flex items-center justify-between border-b border-zinc-100 bg-zinc-50/50 p-4 px-6 dark:border-zinc-800 dark:bg-zinc-800/50">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-sm font-bold uppercase tracking-widest text-zinc-950 dark:text-zinc-100">Edit Task</h3>
          </div>
          <Button variant="ghost" size="icon" onClick={onClose} className="h-8 w-8 text-zinc-400 hover:text-zinc-950 dark:hover:text-zinc-100">
            <X className="h-4 w-4" />
          </Button>
        </div>
        ...
        <form onSubmit={onSaveEditor} className="space-y-6 p-6">
          ...
          <div className="rounded-xl border border-zinc-200 bg-zinc-50/50 p-3 dark:border-zinc-800 dark:bg-zinc-800/40">
            <p className="mb-2 text-xs font-bold uppercase tracking-widest text-zinc-500 dark:text-zinc-400">Checklist / Subtasks</p>
                ...
                <div key={item.id} className="flex items-center gap-2">
                  <button
                    ...
                    className={`flex h-5 w-5 items-center justify-center rounded border text-[10px] ${
                      item.done ? "border-emerald-500 bg-emerald-500 text-white" : "border-zinc-300 bg-white text-zinc-500 dark:border-zinc-600 dark:bg-zinc-950 dark:text-zinc-400"
                    }`}
                  >
                    {item.done ? "✓" : ""}
                  </button>
                  <span className={`flex-1 text-sm ${item.done ? "text-zinc-400 line-through dark:text-zinc-500" : "text-zinc-700 dark:text-zinc-300"}`}>
                    {item.text}
                  </span>
                  <Button
                    type="button"
                    ...
                    className="h-7 w-7 text-zinc-400 hover:bg-red-50 hover:text-red-600 dark:text-zinc-500 dark:hover:bg-red-500/10 dark:hover:text-red-400"
                  >
