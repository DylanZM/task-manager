...
export default function ProfilePage() {
  ...
  const handleSave = async () => {
    await handleUpdateProfile({ display_name: displayName || null }, avatarFile);
    setAvatarFile(null);
  };
            ...
            <div className="relative">
              {avatarUrl ? (
                <img
                  src={avatarUrl}
                  alt="Avatar"
                  className="h-16 w-16 rounded-2xl border-2 border-zinc-200 object-cover"
                />
              ) : (
                <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-zinc-950 text-lg font-bold text-white">
                  {initials || <User className="h-8 w-8" />}
                </div>
              )}
              <label className="absolute -bottom-1 -right-1 flex h-7 w-7 cursor-pointer items-center justify-center rounded-full border-2 border-white bg-zinc-200 text-zinc-600 shadow-sm hover:bg-zinc-300">
